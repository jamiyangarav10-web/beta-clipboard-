#!/usr/bin/env python3
"""Minimal LocalBridge macOS agent shell with local control API."""
import sys
import threading
...
CLIENT = ROOT / "mac" / "mac_client.py"
RUNTIME = AgentRuntime("macos", CLIENT)
...
class AgentWindow:
    ...
    def build(self) -> None:
        ...
        tk.Label(frame, text="LocalBridge", font=("Helvetica", 20, "bold")).pack(anchor="w")
        self.status_label = tk.Label(frame, text="Ready", fg="#13795b", font=("Helvetica", 12, "bold"))
        self.status_label.pack(anchor="w", pady=(4, 20))
        tk.Label(frame, text="Devices", font=("Helvetica", 11, "bold")).pack(anchor="w")
